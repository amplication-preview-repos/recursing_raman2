/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { BacklogItemWhereInput } from "./BacklogItemWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { BacklogItemOrderByInput } from "./BacklogItemOrderByInput";

@ArgsType()
class BacklogItemFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => BacklogItemWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => BacklogItemWhereInput, { nullable: true })
  @Type(() => BacklogItemWhereInput)
  where?: BacklogItemWhereInput;

  @ApiProperty({
    required: false,
    type: [BacklogItemOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [BacklogItemOrderByInput], { nullable: true })
  @Type(() => BacklogItemOrderByInput)
  orderBy?: Array<BacklogItemOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { BacklogItemFindManyArgs as BacklogItemFindManyArgs };
